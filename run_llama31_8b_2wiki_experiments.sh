#!/bin/bash
# Run 2Wiki main experiments with Llama-3.1-8B-Instruct
set -euo pipefail
...
OUTPUT_DIR="results/2wiki_llama31_8b_v2"
EXPERIMENT="all"

mkdir -p "$LOGDIR"
LOG_FILE="${LOGDIR}/logs_${EXPERIMENT}_2wiki_llama31_8b.log"
...
echo "$(date): Starting 2Wiki ${EXPERIMENT} with local model ${MODEL_PATH}..."
$PYTHON -u "$SCRIPT" \
  --experiment "$EXPERIMENT" \
  --model_path "$MODEL_PATH" \
  --output_dir "$OUTPUT_DIR" 2>&1 | tee "$LOG_FILE"
echo "$(date): 2Wiki ${EXPERIMENT} done."
